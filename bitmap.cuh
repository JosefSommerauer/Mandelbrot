...
#define PFC_BITMAP_VERSION_MAJOR          1     /* gro�e �nderungen           */
...
//       assert (((width * 3) % 4 == 0) && "a scanline must end on a 32-bit boundary");
         ...
         m_info_header.size_image = pfc::dword_t (size_image (width, height)); //pfc::dword_t(width*height*4)

		 #if OPTIMISATION_LEVEL >= 7
         	 pfc::check(cudaHostAlloc((void **)m_p_image.getptr(),  (size_t)m_info_header.size_image,  cudaHostAllocMapped));
		 #else
			 pfc::mem_set_ptr (
      ...
      RGB_T * get_image_RGB_3 () {
         return (RGB_T *) get_image ();
      }
   ...
   private:
      ...
      static pfc::word_t const c_bit_count = 32; // use 32 bit for better cashline use
#else
