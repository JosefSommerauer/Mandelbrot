...
// run timer n-times
template <typename timer_t = pfc::default_timer, typename fun_t, typename ...args_t> 
inline typename timer_t::duration timed_run (std::size_t const n, fun_t && fun, args_t && ...args) 
{
   ...
   auto const stop = timer_t::now (); 
	
	return (stop - start) / std::max <std::size_t> (1, n);
}


// run > 0 times
//template <typename timer_t = pfc::default_timer, typename fun_t, typename ...args_t>
//inline typename timer_t::duration timed_run (int const n, fun_t && fun, args_t && ...args)
//{
//   return pfc::timed_run <timer_t, fun_t, args_t...>
...
// run 1 times
template <typename timer_t = pfc::default_timer, typename fun_t, typename ...args_t> 
inline typename timer_t::duration timed_run ( fun_t && fun, args_t && ...args) 
{
...
inline double speedup (
		duration_t const & serial, 
		duration_t const & parallel) 
{
